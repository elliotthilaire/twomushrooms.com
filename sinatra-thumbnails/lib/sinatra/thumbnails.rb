...
module Sinatra
  module Thumbnails
    ...
    def self.convert(src, dest, format)
      ...
      #count number of pixels in format
      pixels = eval(format.split('-')[0].sub('x','*'))

      #return if requesting a high res image
      if pixels.to_i >= 1440000
    ...
    def self.settings
      @@settings ||= { :convert_executable  => 'convert'     ,   
                       ...
                       :watermark           => 'public/watermark.png',
                       :image_path_prefix   => ''             ,
